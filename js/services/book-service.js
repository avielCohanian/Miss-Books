...
//     let book = books.find(book => book.title.toLowerCase().includes(bookName.toLowerCase()))
//     if (book) {
//         console.log('storage', book)
//         return book
//     } else {
...
function searchBook(bookName) {
    const searchRes = utilService.loadFromStorage('baba')
    if (searchRes && searchRes.length) return Promise.resolve(searchRes)

    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`)
        .then(res => {
            const books = res.data.items
            utilService.saveToStorage('baba', books)
            console.log('axios', books)
            return books
        })
}
...
function addGoogleBook(googleBook) {
    ...
    const newBook = {
        ...
        subtitle: (googleBook.volumeInfo.subtitle) ? googleBook.volumeInfo.subtitle : 'No subtitle',
        authors: (googleBook.volumeInfo.authors) ? googleBook.volumeInfo.authors : ['No authors'],
        publishedDate: (googleBook.volumeInfo.publishedDate) ? googleBook.volumeInfo.publishedDate : 'No publishedDate',
        description: (googleBook.volumeInfo.description) ? googleBook.volumeInfo.description : 'No description',
        pageCount: (googleBook.volumeInfo.pageCount) ? googleBook.volumeInfo.pageCount : 'No pageCount',
        categories: (googleBook.volumeInfo.categories) ? googleBook.volumeInfo.categories : ['No categories'],
        thumbnail: googleBook.volumeInfo.imageLinks.thumbnail,
        language: (googleBook.volumeInfo.language) ? googleBook.volumeInfo.language : 'No language',
        listPrice: {
            amount: 19,
            currencyCode: 'ILS',
            isOnSale: true
